...
class BaseOpenEMMA:
    ...
    # [新增] CoT-SC (Chain of Thought with Self-Consistency) 实现
    # 通过生成多个推理链并选择最一致的答案来提高推理准确性
    def getCoT_SC(self, image_path, ego_fut_diff, ego_fut_trajs, ego_his_diff, backbone, num_samples=5):
        """
        ...
        # Select the most consistent decision
        from collections import Counter
        decision_counts = Counter(decisions)
        most_common_decision = decision_counts.most_common(1)[0][0]
        
        # Find the rationale that corresponds to the most common decision
        most_consistent_rationale = rationales[decisions.index(most_common_decision)]
        
        return most_consistent_rationale, most_common_decision

    # [新增] ToT (Tree of Thoughts) 实现
    # 通过探索多个推理路径并评估其质量来选择最佳推理路径
    def getToT(self, image_path, ego_fut_diff, ego_fut_trajs, ego_his_diff, backbone, breadth=3, depth=2):
        """
        Tree of Thoughts (ToT) implementation
        Explores multiple reasoning paths and evaluates their quality
        """
        # Generate initial thoughts (breadth-first)
        initial_thoughts = []
        for i in range(breadth):
            r1 = self.Scenedescription(image_path, backbone)
            r2 = self.get_objects(image_path)
            initial_thought = f"Scene: {r1}\nObjects: {r2}"
            initial_thoughts.append(initial_thought)
        
        # Expand each initial thought (depth-first)
        best_rationale = ""
        best_score = -1
        
        for thought in initial_thoughts:
            # Expand thought for specified depth
            current_thought = thought
            for d in range(depth):
                # Extract objects from the thought
                objects = ""
                if '\nObjects: ' in thought:
                    objects = thought.split('\nObjects: ')[1]
                else:
                    objects = thought
                
                r3 = self.description_criticalobjects(objects, image_path, backbone)
                r4 = self.Metadecision(ego_fut_diff, ego_fut_trajs, ego_his_diff)
                expanded_thought = f"{current_thought}\nBehavior: {r3}\nDecision: {r4}"
                
                # Evaluate thought quality (simplified evaluation)
                score = self.evaluate_thought_quality(expanded_thought, ego_fut_diff, ego_fut_trajs)
                
                if score > best_score:
                    best_score = score
                    best_rationale = expanded_thought
                
                current_thought = expanded_thought
        
        return best_rationale

    # [新增] 推理链质量评估函数
    # 用于评估不同推理链的质量，支持ToT方法的选择机制
    def evaluate_thought_quality(self, rationale, ego_fut_diff, ego_fut_trajs):
        """
        Evaluate the quality of a reasoning chain
        This is a simplified evaluation function
        """
        # Extract decision from rationale
        if "Decision:" in rationale:
            decision = rationale.split("Decision: ")[-1].strip()
        else:
            decision = rationale
        
        # Simple scoring based on consistency with future trajectory
        # In a real implementation, this would be more sophisticated
        score = 0
        if "STOPPED" in decision and np.linalg.norm(ego_fut_diff[-1]) < 0.5:
            score += 1
        elif "ACCELERATING" in decision and np.linalg.norm(ego_fut_diff[-1]) > np.linalg.norm(ego_fut_diff[0]):
            score += 1
        elif "DECELERATING" in decision and np.linalg.norm(ego_fut_diff[-1]) < np.linalg.norm(ego_fut_diff[0]):
            score += 1
    ...
    # [修改] 更新generate_waypoints函数以支持多种推理模式
    # 根据args参数中的reasoning_mode选择相应的推理方法
    def generate_waypoints(self, command, image_path, data=None, backbone=None, args=None):
        ...
        # Use CoT-SC or ToT based on args
        if hasattr(args, 'reasoning_mode') and args.reasoning_mode == 'cot-sc':
            # 使用CoT-SC (Chain of Thought with Self-Consistency) 推理模式
            rationale, decision = self.getCoT_SC(image_path, ego_fut_diff, ego_fut_trajs, ego_his_diff, backbone)
            final_rationale = rationale
        elif hasattr(args, 'reasoning_mode') and args.reasoning_mode == 'tot':
            # 使用ToT (Tree of Thoughts) 推理模式
            rationale = self.getToT(image_path, ego_fut_diff, ego_fut_trajs, ego_his_diff, backbone)
            final_rationale = rationale
        else:
            # Default to original CoT
            # 使用默认的CoT (Chain of Thought) 推理模式
