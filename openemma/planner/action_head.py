...
class PredictedGeometryFusionHead(nn.Module):
    """Predict a geometry descriptor, then fuse it with Qwen and ego features for action chunks."""
    ...
    def __init__(
        ...
        self.geometry_predictor = GeometryPredictor(
    ...
    def forward(
        ...
        geometry_descriptor = self.geometry_predictor(planning_hidden, ego_history_array)
        qwen_embed = self.qwen_projection(planning_hidden)
        ego_flat = ego_history_array.reshape(ego_history_array.shape[0], -1)
        ego_embed = self.ego_encoder(ego_flat)
        geometry_embed = self.geometry_encoder(geometry_descriptor)
        fused = torch.cat([qwen_embed, ego_embed, geometry_embed], dim=-1)
        action_chunk = self.fusion(fused).view(-1, self.chunk_size, self.action_dim)
        return {
...
class PredictedGeometrySequenceFusionHead(nn.Module):
    """Fuse global Qwen/ego context with a predicted per-step geometry sequence."""
    ...
    def __init__(
        ...
        self.per_step_hidden_size = per_step_hidden_size
        self.chunk_size = chunk_size
        self.action_dim = action_dim
        ...
        self.per_step_fusion = nn.Sequential(
            nn.Linear(global_context_dim + geometry_step_embed_dim, per_step_hidden_size),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(per_step_hidden_size, action_dim),
        )
    ...
    def forward(
        ...
        ego_flat = ego_history_array.reshape(ego_history_array.shape[0], -1)
        ego_embed = self.ego_encoder(ego_flat)
        global_context = self.global_context(torch.cat([qwen_embed, ego_embed], dim=-1))
        global_context = global_context.unsqueeze(1).expand(-1, self.chunk_size, -1)
        geometry_embed = self.geometry_step_encoder(geometry_sequence)
        fused = torch.cat([global_context, geometry_embed], dim=-1)
        return self.per_step_fusion(fused)


class DetachedResidualGeometrySequenceFusionHead(nn.Module):
    """Fusion-style base action with a detached geometry-sequence residual correction."""
    ...
    def __init__(
        ...
        self.base_trunk = nn.Sequential(
            nn.Linear(qwen_embed_dim + ego_embed_dim, base_hidden_size),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(base_hidden_size, chunk_size * action_dim),
        )
        self.geometry_step_encoder = nn.Sequential(
            nn.Linear(geometry_sequence_dim, geometry_step_embed_dim),
            nn.GELU(),
        ...
        self.residual_fusion = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(residual_hidden_size, action_dim),
        )
    ...
    def forward(
        ...
        base_action = self.base_trunk(global_features).view(-1, self.chunk_size, self.action_dim)
        global_context = self.global_context(global_features).unsqueeze(1).expand(-1, self.chunk_size, -1)
        geometry_embed = self.geometry_step_encoder(geometry_for_action)
        residual_features = torch.cat([global_context, geometry_embed], dim=-1)
        residual_action = self.residual_fusion(residual_features)
        return base_action + self.residual_scale * residual_action


class CurvatureOnlyDetachedResidualGeometrySequenceFusionHead(nn.Module):
    """Fusion base action with detached geometry residual applied only to curvature."""
    ...
    def forward(
        ...
        if self.action_dim != 2:
            raise ValueError("CurvatureOnlyDetachedResidualGeometrySequenceFusionHead expects action_dim=2.")
        if planning_hidden.ndim != 2:
            raise ValueError("planning_hidden must have shape [B, qwen_hidden_dim].")
        if planning_hidden.shape[-1] != self.qwen_hidden_dim:
            raise ValueError(
        ...
        geometry_embed = self.geometry_step_encoder(geometry_for_action)
        residual_features = torch.cat([global_context, geometry_embed], dim=-1)
        residual_curvature = self.curvature_residual_fusion(residual_features)
...
def build_predicted_geometry_fusion_head(
    qwen_hidden_dim: int = 3584,
    history_steps: int = 10,
    ego_dim: int = 3,
    geometry_descriptor_dim: int = 16,
    ...
    return PredictedGeometryFusionHead(
        qwen_hidden_dim=qwen_hidden_dim,
        history_steps=history_steps,
        ego_dim=ego_dim,
        geometry_descriptor_dim=geometry_descriptor_dim,
...
def build_predicted_geometry_sequence_fusion_head(
    qwen_hidden_dim: int = 3584,
    history_steps: int = 10,
    ego_dim: int = 3,
    qwen_embed_dim: int = 512,
    ...
    return PredictedGeometrySequenceFusionHead(
        qwen_hidden_dim=qwen_hidden_dim,
        history_steps=history_steps,
        ego_dim=ego_dim,
        qwen_embed_dim=qwen_embed_dim,
        ...
        per_step_hidden_size=per_step_hidden_size,
        chunk_size=chunk_size,
        action_dim=action_dim,
...
def build_detached_residual_geometry_sequence_fusion_head(
    qwen_hidden_dim: int = 3584,
    history_steps: int = 10,
    ego_dim: int = 3,
    qwen_embed_dim: int = 512,
    ...
    return DetachedResidualGeometrySequenceFusionHead(
        qwen_hidden_dim=qwen_hidden_dim,
        history_steps=history_steps,
        ego_dim=ego_dim,
        qwen_embed_dim=qwen_embed_dim,
...
def build_curvature_only_detached_residual_geometry_sequence_fusion_head(
    qwen_hidden_dim: int = 3584,
    history_steps: int = 10,
    ego_dim: int = 3,
    qwen_embed_dim: int = 512,
    ...
    return CurvatureOnlyDetachedResidualGeometrySequenceFusionHead(
        qwen_hidden_dim=qwen_hidden_dim,
        history_steps=history_steps,
        ego_dim=ego_dim,
        qwen_embed_dim=qwen_embed_dim,
