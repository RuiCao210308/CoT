...
def main() -> int:
    if torch is None:
        print("[QueryGatedResidualShapeTest] skipped: torch is not installed")
        return 0
    ...
    query_residual_head = QueryGatedResidualHead(
        qwen_hidden_dim=qwen_hidden_dim,
        descriptor_dim=descriptor_dim,
        decoder_dim=32,
    ...
    head = GatedGeometryResidualFusionHead(
        ...
        gated_residual_head=query_residual_head,
        residual_scale=0.1,
    )
