...
from action_head import DecoupledEgoVLAActionHead, action_chunk_l1_loss
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train decoupled EgoVLA action head.")
    ...
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/openemma_decoupled_egovla")
...
def resolve_device(device_arg: str) -> torch.device:
    ...
    if "cuda" in device_arg and not torch.cuda.is_available():
        print("[DecoupledEgoVLATrain] cuda requested but unavailable; falling back to cpu")
        return torch.device("cpu")
...
def train(args):
    if args.batch_size != 1:
        raise ValueError("train_decoupled_egovla_head.py currently supports batch_size=1 only.")
    ...
    print(
        f"[DecoupledEgoVLATrain] loaded_records={len(records)} usable_samples={len(samples)} "
        f"skipped={sum(skipped.values())}"
    )
    if skipped:
        print(f"[DecoupledEgoVLATrain] skipped_reasons={dict(skipped)}")
    print(f"[DecoupledEgoVLATrain] source_action_schema={SOURCE_ACTION_SCHEMA}")
    print(f"[DecoupledEgoVLATrain] train_action_schema={TRAIN_ACTION_SCHEMA}")
    print("[DecoupledEgoVLATrain] target_train[..., 1] = future_action_gt[..., 1] * 100.0")
    print(
        f"[DecoupledEgoVLATrain] loss_weights speed_weight={args.speed_weight} "
        f"curvature_weight={args.curvature_weight}"
    )
    if not samples:
        print("[DecoupledEgoVLATrain] no usable samples")
        return 1
    ...
    for epoch in range(args.epochs):
        ...
        for sample in samples:
            ...
            pred = head(planning_hidden, ego_history)
    ...
    checkpoint = {
        "decoupled_egovla_head_state_dict": head.state_dict(),
        "config": {
            "qwen_hidden_dim": 3584,
            "history_steps": 10,
            "ego_dim": 3,
    ...
    ckpt_path = os.path.join(args.output_dir, "decoupled_egovla_head.pt")
    torch.save(checkpoint, ckpt_path)
    print(f"[DecoupledEgoVLATrain] saved checkpoint: {ckpt_path}")
    return 0
