...
from action_head import OracleGeometryFusionHead, action_chunk_l1_loss
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train oracle-geometry fusion action head.")
    ...
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/openemma_oracle_geometry_fusion_100")
...
def resolve_device(device_arg: str) -> torch.device:
    ...
    if "cuda" in device_arg and not torch.cuda.is_available():
        print("[OracleGeometryFusionTrain] cuda requested but unavailable; falling back to cpu")
        return torch.device("cpu")
...
def train(args):
    if args.batch_size != 1:
        raise ValueError("train_oracle_geometry_fusion_head.py currently supports batch_size=1 only.")
    if args.geometry_dim != ORACLE_GEOMETRY_DESCRIPTOR_DIM:
        raise ValueError(
            f"first-version oracle geometry descriptor dim is {ORACLE_GEOMETRY_DESCRIPTOR_DIM}, "
    ...
    print(
        f"[OracleGeometryFusionTrain] loaded_records={len(records)} usable_samples={len(samples)} "
        f"skipped={sum(skipped.values())}"
    )
    if skipped:
        print(f"[OracleGeometryFusionTrain] skipped_reasons={dict(skipped)}")
    print(f"[OracleGeometryFusionTrain] source_action_schema={SOURCE_ACTION_SCHEMA}")
    print(f"[OracleGeometryFusionTrain] train_action_schema={TRAIN_ACTION_SCHEMA}")
    print(f"[OracleGeometryFusionTrain] waypoint_schema={WAYPOINT_SCHEMA}")
    print("[OracleGeometryFusionTrain] target_train[..., 1] = future_action_gt[..., 1] * 100.0")
    print(
        "[OracleGeometryFusionTrain] oracle_geometry_from_gt_waypoints=True "
        f"purpose={ORACLE_GEOMETRY_PURPOSE}"
    )
    print(
        f"[OracleGeometryFusionTrain] loss_weights speed_weight={args.speed_weight} "
        f"curvature_weight={args.curvature_weight} dt={args.dt} geometry_dim={args.geometry_dim}"
    )
    if not samples:
        print("[OracleGeometryFusionTrain] no usable samples")
        return 1
    ...
    for epoch in range(args.epochs):
        ...
        for sample in samples:
            ...
            pred = head(planning_hidden, ego_history, geometry_descriptor)
    ...
    checkpoint = {
        "oracle_geometry_fusion_head_state_dict": head.state_dict(),
        "config": {
            "oracle_geometry_from_gt_waypoints": True,
            "purpose": ORACLE_GEOMETRY_PURPOSE,
            "qwen_hidden_dim": 3584,
            "history_steps": 10,
            "ego_dim": 3,
            "geometry_descriptor_dim": args.geometry_dim,
    ...
    ckpt_path = os.path.join(args.output_dir, "oracle_geometry_fusion_head.pt")
    torch.save(checkpoint, ckpt_path)
    print(f"[OracleGeometryFusionTrain] saved checkpoint: {ckpt_path}")
    return 0
