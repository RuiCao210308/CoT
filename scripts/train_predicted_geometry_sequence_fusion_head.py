...
from action_head import PredictedGeometrySequenceFusionHead, action_chunk_l1_loss
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train predicted geometry-sequence fusion action head.")
    ...
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/openemma_predicted_geometry_sequence_fusion_100")
...
def resolve_device(device_arg: str) -> torch.device:
    ...
    if "cuda" in device_arg and not torch.cuda.is_available():
        print("[PredictedGeometrySequenceFusionTrain] cuda requested but unavailable; falling back to cpu")
        return torch.device("cpu")
...
def train(args):
    if args.batch_size != 1:
        raise ValueError("train_predicted_geometry_sequence_fusion_head.py currently supports batch_size=1 only.")
    ...
    print(
        f"[PredictedGeometrySequenceFusionTrain] loaded_records={len(records)} usable_samples={len(samples)} "
        f"skipped={sum(skipped.values())}"
    )
    if skipped:
        print(f"[PredictedGeometrySequenceFusionTrain] skipped_reasons={dict(skipped)}")
    print(f"[PredictedGeometrySequenceFusionTrain] source_action_schema={SOURCE_ACTION_SCHEMA}")
    print(f"[PredictedGeometrySequenceFusionTrain] train_action_schema={TRAIN_ACTION_SCHEMA}")
    print(f"[PredictedGeometrySequenceFusionTrain] geometry_sequence_schema={GEOMETRY_SEQUENCE_SCHEMA}")
    print(
        f"[PredictedGeometrySequenceFusionTrain] loss_weights speed_weight={args.speed_weight} "
        f"curvature_weight={args.curvature_weight} "
        f"geometry_sequence_weight={args.geometry_sequence_weight} dt={args.dt}"
    )
    if not samples:
        print("[PredictedGeometrySequenceFusionTrain] no usable samples")
        return 1
    ...
    fusion_head = PredictedGeometrySequenceFusionHead(dropout=0.1).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for sample in samples:
            ...
            pred_action = fusion_head(planning_hidden, ego_history, pred_geometry_sequence)
    ...
    checkpoint = {
        ...
        "predicted_geometry_sequence_fusion_head_state_dict": fusion_head.state_dict(),
        "config": {
    ...
    ckpt_path = os.path.join(args.output_dir, "predicted_geometry_sequence_fusion_head.pt")
    torch.save(checkpoint, ckpt_path)
    print(f"[PredictedGeometrySequenceFusionTrain] saved checkpoint: {ckpt_path}")
    return 0
