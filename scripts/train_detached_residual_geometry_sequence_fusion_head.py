...
from action_head import DetachedResidualGeometrySequenceFusionHead, action_chunk_l1_loss
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train detached residual geometry-sequence fusion action head.")
    ...
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/openemma_detached_residual_geometry_sequence_fusion_trainval_100scenes_e3")
...
def resolve_device(device_arg: str) -> torch.device:
    ...
    if "cuda" in device_arg and not torch.cuda.is_available():
        print("[DetachedResidualGeometrySequenceFusionTrain] cuda requested but unavailable; falling back to cpu")
        return torch.device("cpu")
...
def train(args):
    if args.batch_size != 1:
        raise ValueError("train_detached_residual_geometry_sequence_fusion_head.py currently supports batch_size=1 only.")
    ...
    print(
        f"[DetachedResidualGeometrySequenceFusionTrain] loaded_records={len(records)} usable_samples={len(samples)} "
        f"skipped={sum(skipped.values())}"
    )
    if skipped:
        print(f"[DetachedResidualGeometrySequenceFusionTrain] skipped_reasons={dict(skipped)}")
    print(f"[DetachedResidualGeometrySequenceFusionTrain] source_action_schema={SOURCE_ACTION_SCHEMA}")
    print(f"[DetachedResidualGeometrySequenceFusionTrain] train_action_schema={TRAIN_ACTION_SCHEMA}")
    print(f"[DetachedResidualGeometrySequenceFusionTrain] geometry_sequence_schema={GEOMETRY_SEQUENCE_SCHEMA}")
    print(
        f"[DetachedResidualGeometrySequenceFusionTrain] loss_weights speed_weight={args.speed_weight} "
        f"curvature_weight={args.curvature_weight} "
        f"geometry_sequence_weight={args.geometry_sequence_weight} "
        f"residual_scale={args.residual_scale} detach_geometry_for_action=True dt={args.dt}"
    )
    if not samples:
        print("[DetachedResidualGeometrySequenceFusionTrain] no usable samples")
        return 1
    ...
    fusion_head = DetachedResidualGeometrySequenceFusionHead(
    ...
    checkpoint = {
        ...
        "detached_residual_geometry_sequence_fusion_head_state_dict": fusion_head.state_dict(),
        "config": {
    ...
    ckpt_path = os.path.join(args.output_dir, "detached_residual_geometry_sequence_fusion_head.pt")
    torch.save(checkpoint, ckpt_path)
    print(f"[DetachedResidualGeometrySequenceFusionTrain] saved checkpoint: {ckpt_path}")
    return 0
