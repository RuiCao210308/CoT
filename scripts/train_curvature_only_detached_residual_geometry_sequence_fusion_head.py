...
from action_head import CurvatureOnlyDetachedResidualGeometrySequenceFusionHead, action_chunk_l1_loss
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train curvature-only detached residual geometry-sequence fusion action head."
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/root/autodl-tmp/openemma_curvature_only_detached_residual_geometry_sequence_fusion_trainval_100scenes_e3",
    )
...
def resolve_device(device_arg: str) -> torch.device:
    ...
    if "cuda" in device_arg and not torch.cuda.is_available():
        print("[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] cuda requested but unavailable; falling back to cpu")
        return torch.device("cpu")
...
def train(args):
    if args.batch_size != 1:
        raise ValueError(
            "train_curvature_only_detached_residual_geometry_sequence_fusion_head.py currently supports batch_size=1 only."
        )
    ...
    print(
        f"[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] loaded_records={len(records)} "
        f"usable_samples={len(samples)} skipped={sum(skipped.values())}"
    )
    if skipped:
        print(f"[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] skipped_reasons={dict(skipped)}")
    print(f"[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] source_action_schema={SOURCE_ACTION_SCHEMA}")
    print(f"[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] train_action_schema={TRAIN_ACTION_SCHEMA}")
    print(f"[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] geometry_sequence_schema={GEOMETRY_SEQUENCE_SCHEMA}")
    print(
        f"[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] loss_weights speed_weight={args.speed_weight} "
        ...
        f"detach_geometry_for_action=True residual_target=curvature_only speed_source=base_fusion_action dt={args.dt}"
    )
    if not samples:
        print("[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] no usable samples")
        return 1
    ...
    fusion_head = CurvatureOnlyDetachedResidualGeometrySequenceFusionHead(
        residual_scale=args.residual_scale,
        dropout=0.1,
    ).to(device)
    parameters = list(geometry_predictor.parameters()) + list(fusion_head.parameters())
    ...
    checkpoint = {
        ...
        "curvature_only_detached_residual_geometry_sequence_fusion_head_state_dict": fusion_head.state_dict(),
        "config": {
            ...
            "speed_source": "base_fusion_action",
            "dt": args.dt,
            "dropout": 0.1,
            "lr": args.lr,
            "epochs": args.epochs,
    ...
    ckpt_path = os.path.join(args.output_dir, "curvature_only_detached_residual_geometry_sequence_fusion_head.pt")
    torch.save(checkpoint, ckpt_path)
    print(f"[CurvatureOnlyDetachedResidualGeometrySequenceFusionTrain] saved checkpoint: {ckpt_path}")
    return 0
