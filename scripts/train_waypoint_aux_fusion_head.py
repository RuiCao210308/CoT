...
def parse_args():
    parser = argparse.ArgumentParser(description="Train waypoint-auxiliary fusion action head.")
    ...
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/openemma_waypoint_aux_fusion_100")
    ...
    parser.add_argument("--waypoint_weight", type=float, default=1.0)
...
def resolve_device(device_arg: str) -> torch.device:
    ...
    if "cuda" in device_arg and not torch.cuda.is_available():
        print("[WaypointAuxFusionTrain] cuda requested but unavailable; falling back to cpu")
        return torch.device("cpu")
...
def train(args):
    if args.batch_size != 1:
        raise ValueError("train_waypoint_aux_fusion_head.py currently supports batch_size=1 only.")
    ...
    print(
        f"[WaypointAuxFusionTrain] loaded_records={len(records)} usable_samples={len(samples)} "
        f"skipped={sum(skipped.values())}"
    )
    if skipped:
        print(f"[WaypointAuxFusionTrain] skipped_reasons={dict(skipped)}")
    print(f"[WaypointAuxFusionTrain] source_action_schema={SOURCE_ACTION_SCHEMA}")
    print(f"[WaypointAuxFusionTrain] train_action_schema={TRAIN_ACTION_SCHEMA}")
    print(f"[WaypointAuxFusionTrain] waypoint_schema={WAYPOINT_SCHEMA}")
    print("[WaypointAuxFusionTrain] target_action[..., 1] = future_action_gt[..., 1] * 100.0")
    print(
        f"[WaypointAuxFusionTrain] loss_weights speed_weight={args.speed_weight} "
        f"curvature_weight={args.curvature_weight} waypoint_weight={args.waypoint_weight}"
    )
    if not samples:
        print("[WaypointAuxFusionTrain] no usable samples")
        return 1
    ...
    for epoch in range(args.epochs):
        ...
        for sample in samples:
            ...
            total_loss = action_loss + args.waypoint_weight * waypoint_loss
    ...
    ckpt_path = os.path.join(args.output_dir, "waypoint_aux_fusion_head.pt")
    torch.save(checkpoint, ckpt_path)
    print(f"[WaypointAuxFusionTrain] saved checkpoint: {ckpt_path}")
    return 0
